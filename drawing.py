...
def draw_skeleton(img, pts, w, h):
    ...
    mid_hips = ((p11[0]+p12[0])//2, (p11[1]+p12[1])//2) if p11 and p12 else None
    p5 = safe_pt(pts,5,w,h)
    p6 = safe_pt(pts,6,w,h)
    if mid_hips and p5 and p6:
        mid_sh = ((p5[0]+p6[0])//2, (p5[1]+p6[1])//2)
        draw_line(img, mid_hips, mid_sh, COLOR_BODY)
...
def draw_ball(img, frame, w, h):
    for bb in frame.get("bboxes", []):
        if int(bb.get("class",-1))==2:
            cx=int((bb["x1"]+bb["x2"])/2*w)
            cy=int((bb["y1"]+bb["y2"])/2*h)
            cv2.circle(img,(cx,cy),6,COLOR_BALL,-1)
            return (cx,cy)
    return None
